...
export function buildHintsPrompt(params: {
  ...
  return [
    ...
    '    { "type": "point", "hint": "このブロックに書く内容のヒント" },',
    '    { "type": "reason", "hint": "このブロックに書く内容のヒント" },',
    '    ...',
    '  ],',
    ...
    '```',
    '',
    '【recommendedStructure について】',
    '- テーマと目標文字数に最適な構成をブロックタイプの配列で提案してください。',
    '- typeは以下のいずれか: point（結論）, reason（理由）, example（具体例）, background（背景）, problem（課題）, solution（解決策）, custom（その他）',
    '- 各ブロックに対して、そのブロックで何を書くべきかのヒントも含めてください。',
    '- 順序は上から下への流れを意識してください。',
    '',
    hasExperiences
